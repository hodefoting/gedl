...
#define GEGL_OP_NAME     tdiff
#define GEGL_OP_C_SOURCE tdiff.c
        ...
        for (i=0;i<pixels;i++)
          {
            gint c;
            for (c=0;c<4;c++)
              acc[i*4+c]= fabs(buf[i*4+c]-acc[i*4+c]);
          }
        gegl_buffer_set (output, result, 0, babl_format ("RGBA float"), acc, GEGL_AUTO_ROWSTRIDE);
        gegl_buffer_set (p->acc, result, 0, babl_format ("RGBA float"), buf, GEGL_AUTO_ROWSTRIDE);
        g_free (buf);
        g_free (acc);
      }
